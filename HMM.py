...
class DiscreteHMM(object):
	...
	def forward(self, sequence):
		...
		for k in range(1, len(sequence)): # sequence length
		...
		for i in range(1, N - 1):
			prob += self.alpha[i][len(sequence) - 1] * self.trans_prob[i][N - 1]
	...
	def backward(self):
		N = no_of_states + 2
		self.beta = np.zeros((N, len(sequence)))

		for i in range(1, N - 1):
			self.beta[i][len(sequence) - 1] = self.trans_prob[i][N - 1] * self.emission_prob[i][sequence[len(sequence) - 1]]

		for k in range(len(sequence) - 2, 0, -1):
		...
		def buildGamma(self, P):
			self.gamma = np.zeros((N, N, len(sequence)))
